...
interface RepositoryInterface<T> {
    ...
    fun getById(id: String): T {
        return try {
            col.findOne(Model::id eq id) ?: throw Exception("No item with this id exists")
        } catch (r: Throwable) {
    ...
    fun delete(id: String): Boolean {
        return try {
            col.findOneAndDelete(Model::id eq id) ?: throw Exception("No item with this id exists")
            true
        } catch (t: Throwable) {
    ...
    fun update(entry: Model): T {
        return try {
            ...
            col.findOne(Model::id eq entry.id) ?: throw Exception("No item with this id exists")
